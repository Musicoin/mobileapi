...
class ReleaseController extends BaseController {
  constructor(props) {
    ...
    this.apolloTipTrack = this.apolloTipTrack.bind(this);
  ...
  async apolloTipTrack(parent, args, context, info) {
    ...
    try {
      const musicoins = args.musicoins || 10;
      const trackAddress = args.trackAddress;
      ...
      if (validateResult !== true) {
        return Error('Validation Failed') //this.reject(Request, Response, validateResult);
      }

      // find track
      const release = await this.ReleaseDelegator._loadTrack(trackAddress);
      if (!release) {
        return Error('Track not found') //this.reject(Request, Response, "Track not found: " + trackAddress);
      }

      // find user
      const sender = await this.ReleaseDelegator._loadUser(USER_ACCOUNT);
      if (!sender) {
        return Error('Sender not Found') //this.reject(Request, Response, "sender not found: " + USER_ACCOUNT);
      }
      ...
      const tipCount = release.directTipCount || 0;
      release.directTipCount = tipCount + musicoins;
      //update tipPlays for trendingList algorithm
      release.tipPlays = release.tipPlays + musicoins;
      await release.save();
      this.logger.debug("update tipCount: ", release.directTipCount);
      ...
      return data //this.success(Request, Response, next, data);
    } catch (error) {
