...
class LicenseController {
    ...
    getRandomNew(Request, Response) {
        this.licenseModule.doGetRandomReleases({ ...Request.query, limit: getLimit(Request) }).then( res => {
            Response.send(res)
        });
    }
    ...
    getPppByAddress(Request, Response) {
        ...
        const k = new Promise(function(resolve, reject) {
            LicenseKey.findOne({licenseAddress: Request.params.address}, function(err, licenseKey) {
                if (!licenseKey) return reject({err: "License not found: " + Request.params.address});
        ...
        Promise.join(l, k, function(license, keyResult) {
            ...
            .catch(function(err) {
                console.log(err);
                Response.status(400);
                Response.send({err: "Failed to acquire key or payment was rejected"});
            })
    }
        ...
        this.publishCredentialsProvider.getCredentials()
            .then(function(credentials) {
                return licenseModule.releaseLicense({
                    ...
                    contributors: Request.body.contributors || [{address: Request.body.profileAddress, shares: 1}]
                }, this.publishCredentialsProvider)
            })
            .then(function(tx) {
